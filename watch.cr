...
module Watch
  ...
  def self.watch(glob, full_command, opts = [] of ElementType)
    spawn do
      files = Dir.glob(glob)
...
Watch.watch "./src/**/main.cr", "crystal run src/main.cr", [:on_start]
Watch.watch "./src/**/*.cr", "echo \"nice\"", [:log_changes, :on_start]
Watch.run
